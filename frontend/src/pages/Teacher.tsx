...
const API = import.meta.env.VITE_API || 'https://live-polling-system-backend-seven.vercel.app'
...
export default function Teacher() {
      ...
      <div className="min-h-screen bg-white px-6 py-10">
        <div className="mx-auto max-w-5xl">
          <div className="flex items-center justify-between">
            ...
            <button className="rounded-full bg-brand text-white px-6 py-2" onClick={() => setShowHistory(false)}>Back to Polls</button>
          </div>
          <div className="mt-8">
  ...
  return (
    <div className="min-h-screen bg-white px-6 py-10">
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between">
          <div className="inline-flex items-center gap-2 rounded-full bg-neutral-100 text-neutral-700 px-4 py-1 text-sm font-semibold">
            <span>✨</span>
            <span>Intervue Poll</span>
          </div>
          <button className="rounded-full bg-brand text-white px-6 py-2 flex items-center gap-2" onClick={() => setShowHistory(true)}>
            <span>👁</span>
          ...
          <>
            ...
            <div className="mt-6">
              ...
              <div className="space-y-3">
                {options.map((o, i) => (
                  <div key={i} className="flex items-center gap-4">
                    <div className="flex items-center gap-3 flex-1">
                      <div className="w-7 h-7 rounded-full bg-brand text-white text-xs flex items-center justify-center flex-shrink-0">{i + 1}</div>
                      <input
                ...
                <button
                  className="mt-2 rounded-lg border border-brand-mid text-brand px-4 py-2 text-sm"
                  onClick={() => {
                    setOptions((s) => [...s, ''])
            ...
            <div className="mt-10 flex justify-end">
              <button className="rounded-full bg-brand text-white px-10 py-3" onClick={askQuestion}>Ask Question</button>
            </div>
          </>
        ) : (
          <div className="mt-12">
            <div className="text-lg font-semibold mb-6">Question</div>
            <div className="max-w-3xl rounded-2xl border border-brand-light overflow-hidden">
                ...
                {activePoll.options?.map((o: any, idx: number) => {
                  ...
                  return (
                    <div key={o.id} className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-brand text-white text-sm flex items-center justify-center flex-shrink-0 font-semibold">{o.text.charAt(0).toUpperCase()}</div>
                      <div className="flex-1">
                        <div className="relative w-full h-10 bg-gray-200 rounded-lg overflow-hidden border border-gray-300">
                          <div className="absolute left-0 top-0 h-full bg-brand" style={{ width: `${pct}%` }} />
            ...
            <div className="mt-8 flex justify-center">
              <button 
                className="rounded-full bg-brand text-white px-8 py-3 font-semibold"
                onClick={newQuestion}
              >
                + Ask a new question
              </button>
